...
class ClassPieEcharts extends React.Component {
    state={
        data:[],
    }
    componentWillMount() {
        axios.post('/api/worktt/task/countByCategory.action')
            .then((response)=>{
                let data=[];
                ...
                myChart.setOption({
                    ...
                    legend: {
                        orient: 'vertical',
                        left: 'left',
                        data: response.data.datas.category,
                    },
                    series : [
                        {
                            name: '类别',
    ...
    render() {
        return (
            <div id="main01" style={{width: 510, height: 350, padding: 40,}}/>
        );
    }
}
export default ClassPieEcharts;
