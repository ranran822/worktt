...
class ManagerPieEcharts extends Component {
    state={
        data:[],
    }
    // constructor(props){
    ...
    // }
    componentWillMount() {
        axios.post('/api/worktt/task/TaskByOwner.action')
            .then((response)=>{
                let data=[];
                ...
                myChart.setOption({
                    ...
                    legend: {
                        orient: 'vertical',
                        left: 'left',
                        data: response.data.datas.emp_name,
                    },
                    series : [
                        {
                            name: '项目负责人',
    ...
    render() {
        return (
            <div id="main03" style={{width: 510, height: 350, padding: 40}}/>
        );
    }
}
export default ManagerPieEcharts;
